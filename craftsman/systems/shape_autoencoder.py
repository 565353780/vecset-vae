...
class ShapeAutoEncoderSystem(BaseSystem):
    ...
    def configure(self):
        ...
        self.shape_model = craftsman.find(self.cfg.shape_model_type)(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out = self(batch, "val")
        try:
            save_slice_dir = self.get_save_path(
                f"it{self.true_global_step}/{os.path.basename(batch['uid'][0])}".replace(
                    ".npz", ""
                )
            )  # turn on
            # save_slice_dir = ''  # turn off
            mesh_v_f, has_surface = self.shape_model.extract_geometry_by_diffdmc(
                out["latents"], octree_depth=9, save_slice_dir=save_slice_dir
            )
            file_path = f"it{self.true_global_step}/{os.path.basename(batch['uid'][0])}".replace(
                ".npz", ".obj"
            )
            if not os.path.exists(file_path):
                self.save_mesh(file_path, mesh_v_f[0][0], mesh_v_f[0][1])
        ...
        overall_accuracy = (overall_pred == overall_labels).float().sum(
            dim=1
        ) / overall_labels.shape[1]
        ...
        coarse_accuracy = (coarse_pred == coarse_labels).float().sum(
            dim=1
        ) / coarse_labels.shape[1]
        ...
        sharp_accuracy = (sharp_pred == sharp_labels).float().sum(
            dim=1
        ) / sharp_labels.shape[1]
        ...
        self.log(
            f"val/overall_accuracy", overall_accuracy, sync_dist=True, on_epoch=True
        )
